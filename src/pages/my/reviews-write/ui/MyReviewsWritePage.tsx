...
export default async function MyReviewsWritePage() {
  const session = await auth();
  if (!session?.user?.id) {
    redirect(getLoginRedirectPath('/my/reviews/write'));
  }
  ...
  queryClient.prefetchQuery({
    queryKey: orderQueryKeys.paged('review', 1, 5),
    queryFn: () => getOrdersResultByMode(userId, 'review', 1, 5),
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  });
